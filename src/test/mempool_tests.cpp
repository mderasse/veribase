...
BOOST_AUTO_TEST_CASE(MempoolSizeLimitTest)
{
    ...
    BOOST_CHECK_EQUAL(pool.GetMinFee().GetFeePerK(), llround((maxFeeRateRemoved.GetFeePerK() + 1000)/2.0));
    // ... then feerate should drop 1/2 each halflife

    SetMockTime(42 + 2*CTxMemPool::ROLLING_FEE_HALFLIFE + CTxMemPool::ROLLING_FEE_HALFLIFE/2);
    BOOST_CHECK_EQUAL(pool.GetMinFee().GetFeePerK(), llround((maxFeeRateRemoved.GetFeePerK() + 1000)/4.0));
