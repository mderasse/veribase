...
bool Consensus::CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight, CAmount& txfee, const Consensus::Params& params, uint64_t nMoneySupply)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); ++i) {
        ...
        // XXX: to uncoment after coin implementation
        ...
        // vericonomy: check transaction timestamp
        if (coin.nTime > tx.nTime)
            return state.Invalid(ValidationInvalidReason::CONSENSUS, false, REJECT_INVALID, "bad-txns-spent-too-early", strprintf("%s : transaction timestamp earlier than input transaction", __func__));
