...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const Consensus::Params& params)
{
    if (pindexLast->nHeight <= 2)
        return params.powLimit.GetCompact(); // first few blocks
    ...
    if (bnNew > CBigNum(ArithToUint256(params.powLimit)))
        bnNew = CBigNum(ArithToUint256(params.powLimit));
...
bool CheckProofOfWork(uint256 hash, unsigned int nBits, const Consensus::Params& params)
{
    ...
    // Check range
    if (bnTarget <= 0 || bnTarget > CBigNum(ArithToUint256(params.powLimit)))
